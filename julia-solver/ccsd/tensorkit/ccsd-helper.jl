...
function calculate_residual(T2, R2u, R2)
    ...
    f_vv = TensorMap(deserialize("f_vv.jlbin"), ℝ^nv, ℝ^nv)
    f_oo = TensorMap(deserialize("f_oo.jlbin"), ℝ^no, ℝ^no)
...
function calculate_ECCSD()
    ...
    T2_vvoo = nothing
    g_oovv = nothing
    f_ov = TensorMap(deserialize("f_ov.jlbin"),ℝ^no,ℝ^nv)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"),ℝ^nv,ℝ^no)
    @tensor Z += 2 * f_ov[ i_1, a_1 ] * T1_vo[ a_1, i_1 ]
    T1_vo = nothing
    f_ov = nothing
    I_ov = TensorMap(zeros(Float64, no, nv), ℝ^no,ℝ^nv)
    g_oovv = TensorMap(deserialize("g_oovv.jlbin"),ℝ^no ⊗ ℝ^no,ℝ^nv ⊗ ℝ^nv)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"),ℝ^nv,ℝ^no)
...
function PerformDiis(p, T2_storage, R2_storage,T1_storage,R1_storage,dummycall)
    ...
    t2 = TensorMap(zeros(Float64,nv,nv,no,no),ℝ^nv ⊗ ℝ^nv , ℝ^no ⊗ ℝ^no)
    t1 = TensorMap(zeros(Float64,nv,no),ℝ^nv,ℝ^no)
    for i in 1:p
...
function calc_R2()
    ...
    I_vvoo = TensorMap(zeros(Float64, nv, nv, no, no), ℝ^nv ⊗ ℝ^nv, ℝ^no ⊗ ℝ^no)
    g_oooo = TensorMap(deserialize("g_oooo.jlbin"), ℝ^no ⊗ ℝ^no, ℝ^no ⊗ ℝ^no)
    INTpp_vvoo = TensorMap(deserialize("INTpp_vvoo.jlbin"), ℝ^nv ⊗ ℝ^nv, ℝ^no ⊗ ℝ^no)
    ...
    g_vvoo = nothing
    g_vvvo = TensorMap(deserialize("g_vvvo.jlbin"), ℝ^nv ⊗ ℝ^nv, ℝ^nv ⊗ ℝ^no)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    T2_vvoo = nothing
    g_voov = nothing
    g_vooo = TensorMap(deserialize("g_vooo.jlbin"), ℝ^nv ⊗ ℝ^no, ℝ^no ⊗ ℝ^no)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    T2_vvoo = nothing
    f_oo = nothing
    I_oo = TensorMap(zeros(Float64, no, no), ℝ^no, ℝ^no)
    g_oovo = TensorMap(deserialize("g_oovo.jlbin"), ℝ^no ⊗ ℝ^no, ℝ^nv ⊗ ℝ^no)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    T2_vvoo = nothing
    I_oo = nothing
    I_vovo = TensorMap(zeros(Float64, nv, no, nv, no), ℝ^nv ⊗ ℝ^no, ℝ^nv ⊗ ℝ^no)
    g_oovv = TensorMap(deserialize("g_oovv.jlbin"), ℝ^no ⊗ ℝ^no, ℝ^nv ⊗ ℝ^nv)
    T2_vvoo = TensorMap(deserialize("T2_vvoo.jlbin"), ℝ^nv ⊗ ℝ^nv, ℝ^no ⊗ ℝ^no)
    ...
    T2_vvoo = nothing
    I_vovo = nothing
    I_oooo = TensorMap(zeros(Float64, no, no, no, no), ℝ^no ⊗ ℝ^no, ℝ^no ⊗ ℝ^no)
    g_oovo = TensorMap(deserialize("g_oovo.jlbin"), ℝ^no ⊗ ℝ^no, ℝ^nv ⊗ ℝ^no)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    I_vovo = nothing
    I_vovo = TensorMap(zeros(Float64, nv, no, nv, no), ℝ^nv ⊗ ℝ^no, ℝ^nv ⊗ ℝ^no)
    g_vovv = TensorMap(deserialize("g_vovv.jlbin"), ℝ^nv ⊗ ℝ^no, ℝ^nv ⊗ ℝ^nv)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    T2_vvoo = nothing
    I_vovo = nothing
    I_vooo = TensorMap(zeros(Float64, nv, no, no, no), ℝ^nv ⊗ ℝ^no, ℝ^no ⊗ ℝ^no)
    g_voov = TensorMap(deserialize("g_voov.jlbin"), ℝ^nv ⊗ ℝ^no, ℝ^no ⊗ ℝ^nv)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
    ...
    I_vooo = nothing
    I_vooo = TensorMap(zeros(Float64, nv, no, no, no), ℝ^nv ⊗ ℝ^no, ℝ^no ⊗ ℝ^no)
    I_oovo = TensorMap(zeros(Float64, no, no, nv, no), ℝ^no ⊗ ℝ^no, ℝ^nv ⊗ ℝ^no)
    g_oovv = TensorMap(deserialize("g_oovv.jlbin"), ℝ^no ⊗ ℝ^no, ℝ^nv ⊗ ℝ^nv)
    T1_vo = TensorMap(deserialize("T1_vo.jlbin"), ℝ^nv, ℝ^no)
...
function calc_R1()
    nv = deserialize("nv.jlbin")::Int64
    no = deserialize("no.jlbin")::Int64
    I_vo = TensorMap(zeros(Float64, nv, no), ℝ^nv, ℝ^no)
    f_ov = TensorMap(deserialize("f_ov.jlbin"), ℝ^no, ℝ^nv)
    T2_vvoo = TensorMap(deserialize("T2_vvoo.jlbin"), ℝ^nv ⊗ ℝ^nv, ℝ^no ⊗ ℝ^no)
    @tensor I_vo[a_1, i_1] += -1 * f_ov[i_2, a_2] * T2_vvoo[a_1, a_2, i_2, i_1]
    T2_vvoo = nothing
    f_ov = nothing
    f_vo = TensorMap(deserialize("f_vo.jlbin"), ℝ^nv, ℝ^no)
...
#     f_vv = TensorMap(deserialize("f_vv.jlbin")::Array{Float64,2}, ℝ^nv, ℝ^nv)
...
#     f_oo = TensorMap(deserialize("f_oo.jlbin")::Array{Float64,2}, ℝ^no, ℝ^no)
