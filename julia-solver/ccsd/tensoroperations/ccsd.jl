# This version reads and writes residuals from disk instead of passing it around as an argument.
...
function ccd_by_hand(maxitr)
    ...
    normtol=1.0e-8
    # display(T2) # Using MP2 amplitudes
    e_new = calculate_ECCD(T2)
    e_old = copy(0.0)
    earr = []
    etol = 1.0e-10
    ...
    for i in 1:maxitr
        ...
        # display(R2)
        # @printf("%.8f",R2)
        T2 = update_amplitudes(T2,R2,Scaled_R2)
        # println("Current ||T2|| = $(SquareNorm(T2))")
        # display(T2)
